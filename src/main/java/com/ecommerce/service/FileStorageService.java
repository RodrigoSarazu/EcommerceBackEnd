...
import com.ecommerce.exception.MyFileNotFoundException;
...
public class FileStorageService {
	...
	private Path getFileStorageLocation(String folderName) {
		Path fileStorageLocation = Paths.get(fileStorageProperties.getUploadDir() + "/" + folderName).toAbsolutePath()
				.normalize();
		try {
	...
	public Resource loadResource(String completeFileName) {
		...
		try {
			...
			throw new MyFileNotFoundException(
					"Ha ocurrido un error al intentar acceder al archivo: " + completeFileName, e);
		}
	}
}
